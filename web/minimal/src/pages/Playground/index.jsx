...
const Playground = () => {
  ...
  // 接收极简首页携带的问题：模型就绪后自动发送
  const pendingFromHomeRef = useRef(false);
  useEffect(() => {
    if (pendingFromHomeRef.current) return;
    let pendingPrompt = null;
    try {
      pendingPrompt = sessionStorage.getItem('minimal_pending_prompt');
    } catch (e) {
      pendingPrompt = null;
    }
    if (!pendingPrompt) return;
    // 等待模型列表加载完成且当前模型为有效值，避免用无效的默认模型发送
    if (!models || models.length === 0) return;
    if (!inputs.model || !models.some((m) => m.value === inputs.model)) return;
    pendingFromHomeRef.current = true;
    try {
      sessionStorage.removeItem('minimal_pending_prompt');
    } catch (e) {
      // ignore
    }
    onMessageSend(pendingPrompt);
